...
function render(keepScroll=true){
 ...
 const css=doc.createElement('style');css.textContent=styles;doc.head.append(css);
 const editingCSS=doc.createElement('style');editingCSS.textContent='[data-edit-key]{cursor:text}[data-edit-key]:hover,[data-image-key]:hover{outline:2px dashed #3cb675;outline-offset:4px}[data-image-key]{cursor:pointer}.editing-selected{outline:3px solid #2ab36b!important;outline-offset:4px}a,button{cursor:pointer}.js-motion .reveal{opacity:1!important;transform:none!important}';doc.head.append(editingCSS);
...
function select(type,key,el){selection={type,key};$('text-panel').hidden=type!=='text';$('image-panel').hidden=type!=='image';if(type==='text'){const [g,k]=key.split('.');$('selection-title').textContent='Editar texto';$('selection-help').textContent=g==='comun'?'Este texto se comparte entre todas las páginas.':'Edita este texto y revisa el resultado a la derecha.';$('text-value').value=value(g,k);}else{const video=/\.mp4$/i.test(data.media[key]);$('selection-title').textContent=video?'Cambiar video':key==='logo_relieve'?'Cambiar logo':'Cambiar imagen';$('selection-help').textContent='Este archivo se actualizará en todos los lugares donde se utiliza.';$('image-preview').hidden=video;$('image-preview').src=video?'':mediaURL(data.media[key]);$('file').accept=video?'video/mp4':'image/png,image/jpeg,image/webp,image/gif';$('file-label').textContent=video?'Seleccionar MP4':'Seleccionar imagen';$('media-note').textContent=video?'MP4 de hasta 20 MiB. Cambia su portada en Todas las fotos, logo y video.':key==='logo_relieve'?'Usa PNG transparente. El diseño muestra el logo en blanco.':'PNG, JPG, WebP o GIF. Hasta 8 MiB.';$('file').value='';}highlight();}
...
$('login-form').addEventListener('submit',async e=>{e.preventDefault();token=$('token').value.trim();$('token').value='';$('connect').disabled=true;$('login-status').textContent='Abriendo la versión más reciente de tu página…';try{await load();$('login').hidden=true;$('workspace').hidden=false;status('Selecciona un texto o una imagen para editar.');}catch(e){token='';$('login-status').textContent=e.message;}finally{$('connect').disabled=false;}});
